...
public class CurrentInfo {
    ...
    private static final String BATT_CURRENT_ADC =
    ...
    private static final String CURRENT_SM705 = "/sys/class/power_supply/main_battery/current_now";
    ...
    public Long getCurrentValue() {
        ...
        if (BUILD_MODEL.contains("sgh-i337") || BUILD_MODEL.contains("gt-i9505")
                || BUILD_MODEL.contains("sch-i545") || BUILD_MODEL.contains("find 5")
                || BUILD_MODEL.contains("sgh-m919") || BUILD_MODEL.contains("sgh-i537")
        ...
        if (BUILD_MODEL.contains("desire hd") || BUILD_MODEL.contains("desire z")) {
            ...
            if (f.exists()) return getCurrentValue(f, false);
        }
        // SM705
        if (BUILD_MODEL.contains("sm705")) {
            f = new File(CURRENT_SM705);
            if (f.exists()) {
                return getCurrentValue(f, false);
            }
        }
        ...
        if (f.exists()) return getSMemValue();

        // nexus one,meizu
        f = new File(CURRENT_NOW);
        if (f.exists()) return getCurrentValue(f, true);
    ...
    public Long getCurrentValue(File file, boolean convertToMillis) {
        ...
        if (line != null) {
            ...
            if (convertToMillis) value = value / 1000;
        }
        return value;
    }
}

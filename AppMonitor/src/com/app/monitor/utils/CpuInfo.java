...
import com.app.monitor.core.InfoModel;
...
public class CpuInfo {
    ...
    private void readTotalCpuStat() {
        try {
            ...
            while ((null != (line = cpuInfo.readLine())) && line.startsWith("cpu")) {
                ...
                totalCpu.add(Long.parseLong(toks[1]) + Long.parseLong(toks[2])
    ...
    /**
     * reserve used ratio of process CPU and total CPU, meanwhile collect network traffic.
     * 
     * @return network traffic ,used ratio of process CPU and total CPU in certain interval
     */
    public ArrayList<String> getCpuRatioInfo(String totalBatt, String currentBatt,
            String temperature, String voltage, String fps, boolean isRoot) {
        ...
        if (isInitialStatics) {
            ...
            if (null != totalCpu2 && totalCpu2.size() > 0) {
                processCpuRatio =
                        fomart.format(100 * ((double) (processCpu - processCpu2) / ((double) (totalCpu
                                .get(0) - totalCpu2.get(0)))));
                for (int i = 0; i < (totalCpu.size() > totalCpu2.size()
                        ? totalCpu2.size()
                        : totalCpu.size()); i++) {
                    String cpuRatio = "0.00";
                    if (totalCpu.get(i) - totalCpu2.get(i) > 0) {
                        cpuRatio =
                                fomart.format(100 * ((double) ((totalCpu.get(i) - idleCpu.get(i)) - (totalCpu2
                                        .get(i) - idleCpu2.get(i))) / (double) (totalCpu.get(i) - totalCpu2
                                        .get(i))));
                    }
            ...
            if (isPositive(processCpuRatio) && isPositive(totalCpuRatio.get(0))) {
                ...
                if (isRoot) {
                    ...
                    heapData =
                            heapArray[0][1] + "/" + heapArray[0][0] + Constants.COMMA
                                    + heapArray[1][1] + "/" + heapArray[1][0] + Constants.COMMA;
                }
                // Log.e("yjl", "时间:" + mDateTime2);
                // Log.e("yjl", "栈顶activity：" + ProcessInfo.getTopActivity(context));
                // Log.e("yjl", "应用占用内存堆:" + heapData);
                // Log.e("yjl", "应用占用内存pMemory:" + pMemory);
                // Log.e("yjl", "应用cpu占比:" + percent);
                // Log.e("yjl", "剩余内存:" + fMemory);
                // Log.e("yjl", "processCpuRatio：" + processCpuRatio);
                // Log.e("yjl", "cpu总使用率totalCpuBuffer:" + totalCpuBuffer.toString());
                // Log.e("yjl", "trafValue:" + trafValue);
                // Log.e("yjl", "流量:" + totalBatt);
                // Log.e("yjl", "电量:" + currentBatt);
                // Log.e("yjl", "温度:" + temperature);
                // Log.e("yjl", "电压:" + voltage);
                // Log.e("yjl", "fps:" + fps);
                InfoModel infoModel = new InfoModel();
                infoModel.setmDateTime(mDateTime2);
                infoModel.setTopActivity(ProcessInfo.getTopActivity(context));
                infoModel.setHeapData(heapData);
                infoModel.setpMemory(pMemory);
                infoModel.setPercent(percent);
                infoModel.setfMemory(fMemory);
                infoModel.setProcessCpuRatio(processCpuRatio);
                infoModel.setTotalCpuBuffer(totalCpuBuffer);
                infoModel.setTrafValue(trafValue);
                infoModel.setTotalBatt(totalBatt);
                infoModel.setCurrentBatt(currentBatt);
                infoModel.setTemperature(temperature);
                infoModel.setVoltage(voltage);
                infoModel.setFps(fps);
                MonitorService.writeToFile(mDateTime2 + Constants.COMMA
                        + ProcessInfo.getTopActivity(context) + Constants.COMMA + heapData
                        + pMemory + Constants.COMMA + percent + Constants.COMMA + fMemory
                        + Constants.COMMA + processCpuRatio + Constants.COMMA
                        + totalCpuBuffer.toString() + trafValue + Constants.COMMA + totalBatt
                        + Constants.COMMA + currentBatt + Constants.COMMA + temperature
                        + Constants.COMMA + voltage + Constants.COMMA + fps + Constants.LINE_END);
                totalCpu2 = (ArrayList<Long>) totalCpu.clone();
                processCpu2 = processCpu;
                idleCpu2 = (ArrayList<Long>) idleCpu.clone();
                ...
                cpuUsedRatio.add("" + totalBatt);
                cpuUsedRatio.add("" + temperature);

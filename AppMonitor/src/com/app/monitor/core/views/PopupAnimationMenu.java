...
public class PopupAnimationMenu implements CircleButton.OnCircleButtonClickListener {
	...
	private CircleButton[] circleButtons = new CircleButton[MAX_CIRCLE_BUTTON_NUMBER]; // this
																						// Array
																						// reference
																						// cannot
																						// never
    ...
    private void placeDots() {
        ...
        FrameLayout.LayoutParams[] ps = PlaceParamsFactory.getDotParams(
                placeType,
                animationLayout.getWidth(),
                animationLayout.getHeight(),
                dotWidth,
                dotHeight
        );
	...
	public void shoot() {
		//if (buttonType.equals(ButtonType.CIRCLE)) {
			for (int i = 0; i < buttonNum; i++) {
				...
				circleButtons[i].getImageView().setScaleType(
						subButtonImageScaleType);
...
//									.getAnimatedFraction());
		...
		if (placeType.SHARE_3_1.v <= placeType.v
				&& placeType.v <= PlaceType.SHARE_9_2.v) {
			ObjectAnimator shareLinesAnimator = ObjectAnimator.ofFloat(
					shareLines, "offset", 1f, 0f).setDuration(
	...
	public void setHideAnimation(final View dot, final View button,
		...
		if (view1 != null) {
			ObjectAnimator alphaAnimator1 = ObjectAnimator.ofFloat(view1,
					"alpha", 1f, 0f).setDuration(duration);
		...
		if (view2 != null) {
			ObjectAnimator alphaAnimator2 = ObjectAnimator.ofFloat(view2,
					"alpha", 1f, 0f).setDuration(duration);
	...
	public void lightAnimationLayout() {
		...
		if (placeType.SHARE_3_1.v <= placeType.v
				&& placeType.v <= PlaceType.SHARE_9_2.v) {
			ObjectAnimator shareLinesAnimator = ObjectAnimator.ofFloat(
					shareLines, "offset", 0f, 1f).setDuration(
					duration + delay * (buttonNum - 1));
			shareLinesAnimator.setStartDelay(0);
			shareLinesAnimator.start();
		}

...
@SuppressLint("NewApi")
public class PopupMenuView extends BaseFloatView
        implements
    ...
    public void initViews(int layout) {
        ...
        createAnimLayout();
        rootView = animationLayout;

    }

    public void initDatas(Drawable[] drawables, String[] strings, int[][] colors) {
        ...
        if (buttonType.equals(ButtonType.CIRCLE)) {
            ...
            if (isInList || MEMORY_OPTIMIZATION) {
                // store the drawables, THEN we will build the buttons when
                // create them
    ...
    private void placeDots() {
        ...
        if (placeType.SHARE_3_1.v <= placeType.v && placeType.v <= PlaceType.SHARE_9_2.v) {
            ...
            // = new FrameLayout.LayoutParams(frameLayout.getWidth(),
        ...
        if (buttonType.equals(ButtonType.CIRCLE)) {
            ...
            if (showOrderType.equals(OrderType.DEFAULT)) {
                for (int i = 0; i < buttonNum; i++) {
                    ...
                    startLocations[i][0] = mPositon[0];// (buttonWidth -
                                                       // dots[i].getWidth()) /
                                                       // 2;
                    startLocations[i][1] = mPositon[1];// (buttonWidth -
                                                       // dots[i].getHeight())
                                                       // / 2;
    ...
    public void onClick(int index) {
        ...
        if (mOnMonitorListener != null) mOnMonitorListener.handlerClick(index);

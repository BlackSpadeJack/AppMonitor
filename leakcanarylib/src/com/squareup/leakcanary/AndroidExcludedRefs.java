...
/**
 * This class is a work in progress. You can help by reporting leak traces that seem to be caused by
 * the Android SDK, here: https://github.com/square/leakcanary/issues/new
 * 
 ...
 * 
 * Most app developers should use {@link #createAppDefaults()}. However, you can also pick the leaks
 * you want to ignore by creating an {@link EnumSet} that matches your needs and calling
...
public enum AndroidExcludedRefs {
    ...
    SPAN_CONTROLLER(SDK_INT <= KITKAT) {
        ...
        void add(ExcludedRefs.Builder excluded) {
            ...
            // SpannableStringBuilder. The SpannableString constructor does not drop NoCopySpan
            // spans.
            // So we end up with a saved state that holds a reference to the textview and therefore
            // the
    ...
    MEDIA_SESSION_LEGACY_HELPER__SINSTANCE(SDK_INT == LOLLIPOP) {
        @Override
        void add(ExcludedRefs.Builder excluded) {
            // MediaSessionLegacyHelper is a static singleton that is lazily instantiated and keeps
            // a
    ...
    TEXT_LINE__SCACHED(SDK_INT < LOLLIPOP_MR1) {
        @Override
        void add(ExcludedRefs.Builder excluded) {
            // TextLine.sCached is a pool of 3 TextLine instances. TextLine.recycle() has had at
            // least two
            // bugs that created memory leaks by not correctly clearing the recycled TextLine
            // instances.
            ...
            // Hack: to fix this, you could access TextLine.sCached and clear the pool every now and
            // then
    ...
    BLOCKING_QUEUE(SDK_INT < LOLLIPOP) {
        ...
        void add(ExcludedRefs.Builder excluded) {
            ...
            // So when a HandlerThread becomes idle, it keeps a local reference to the last message
            // it
            // received. That message then gets recycled and can be used again.
            // As long as all messages are recycled after being used, this won't be a problem,
            // because
            // there references are cleared when being recycled.
            // However, dialogs create template Message instances to be copied when a message needs
            // to be
            // sent. These Message templates holds references to the dialog listeners, which most
            // likely
            // leads to holding a reference onto the activity in some way. Dialogs never recycle
            // their
            // template Message, assuming these Message instances will get GCed when the dialog is
            // GCed.
            ...
            // Hack: to fix this, you could post empty messages to the idle handler threads from
            // time to
    ...
    INPUT_METHOD_MANAGER__ROOT_VIEW(SDK_INT >= ICE_CREAM_SANDWICH_MR1 && SDK_INT <= LOLLIPOP_MR1) {
        @Override
        void add(ExcludedRefs.Builder excluded) {
            // The singleton InputMethodManager is holding a reference to mCurRootView long after
            // the
    ...
    DEVICE_POLICY_MANAGER__SETTINGS_OBSERVER(MOTOROLA.equals(MANUFACTURER) && SDK_INT == KITKAT) {
        ...
        void add(ExcludedRefs.Builder excluded) {
            if (MOTOROLA.equals(MANUFACTURER) && SDK_INT == KITKAT) {
                ...
                // instead of
                // extracting the application context. In this Motorola build, DevicePolicyManager
                // has an
                // inner SettingsObserver class that is a content observer, which is held into
                // memory
                // by a binder transport object.
                excluded.instanceField("android.app.admin.DevicePolicyManager$SettingsObserver",
                        "this$0");
            }
        }
    },
    ...
    CLIPBOARD_UI_MANAGER__SINSTANCE(SAMSUNG.equals(MANUFACTURER) && SDK_INT >= KITKAT
            && SDK_INT <= LOLLIPOP) {
        @Override
        void add(ExcludedRefs.Builder excluded) {
    ...
    BUBBLE_POPUP_HELPER__SHELPER(LG.equals(MANUFACTURER) && SDK_INT >= KITKAT
        ...
        @Override
        void add(ExcludedRefs.Builder excluded) {
            // A static helper for EditText "bubble popups" leaks a reference to the latest focused
            // view.
    ...
    RESOURCES__MCONTEXT(SAMSUNG.equals(MANUFACTURER) && SDK_INT == KITKAT) {
        ...
        void add(ExcludedRefs.Builder excluded) {
            ...
            // Here we have ZygoteInit.mResources (static field) holding on to a Resources instance
            // that
    ...
    AUDIO_MANAGER__MCONTEXT_STATIC(SAMSUNG.equals(MANUFACTURER) && SDK_INT == KITKAT) {
        @Override
        void add(ExcludedRefs.Builder excluded) {
            // Samsung added a static mContext_static field to AudioManager, holds a reference to
            // the
            // activity.
    ...
    MAIN {
        ...
        void add(ExcludedRefs.Builder excluded) {
            ...
            // references
            // for long. If this is on the shortest path, it's probably that there's a longer path
            // with
            // a real leak.
